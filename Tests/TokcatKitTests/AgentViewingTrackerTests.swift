...
final class AgentViewingTrackerTests: XCTestCase {
    func testAgentFocusDwellAndSwitchingDoesNotAcknowledgeOtherAppsOrFutureCompletions() {
        ...
        XCTAssertNil(tracker.viewedCompletions(bundleIdentifier: "com.tencent.workbuddy.mac", now: now.addingTimeInterval(2.99)))
        let viewed = tracker.viewedCompletions(bundleIdentifier: "com.tencent.workbuddy.mac", now: now.addingTimeInterval(3))
        ...
        XCTAssertNil(tracker.viewedCompletions(bundleIdentifier: "com.openai.codex", now: now.addingTimeInterval(23.99)))
        XCTAssertEqual(tracker.viewedCompletions(bundleIdentifier: "com.openai.codex", now: now.addingTimeInterval(24))?.source, .codexCLI)
        XCTAssertNil(tracker.viewedCompletions(bundleIdentifier: "com.apple.Terminal", now: now.addingTimeInterval(25)))
        ...
        XCTAssertEqual(tracker.viewedCompletions(bundleIdentifier: "com.openai.codex", now: now.addingTimeInterval(33))?.through, now.addingTimeInterval(30))
    }
}
